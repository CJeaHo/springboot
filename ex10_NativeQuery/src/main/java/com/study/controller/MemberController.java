...
public class MemberController {
	...
	public String selectMembers1(@RequestParam("name") String name, Model model) {
		name = "%" + name + "%";
		model.addAttribute("m1",memberService.selectMembers1(name)); 
	    return "select_name_jsql1";
	}

	@GetMapping("/selectByNameLike2")
	public String selectMembers2(@RequestParam("name") String name, Model model) {
		name = "%" + name + "%";
		Sort sort = Sort.by(Sort.Order.asc("id"));
		model.addAttribute("m2",memberService.selectMembers2(name, sort));
		return "select_name_jsql2";
	}
	
	@GetMapping("/selectByNameLike3")
	public String selectMembers3(@RequestParam("name") String name, 
	    ...
	    return "select_name_jsql3";
	}

	
	
	@GetMapping("/selectByNameLike4")
	public String selectMembers4(@RequestParam("name") String name, Model model) {
		name = "%" + name + "%";
		model.addAttribute("m3",memberService.selectMembers4(name));
		return "select_name_jsql4";
	}
	/*
	@GetMapping("/selectByNameLike1")
	public String selectByNameLike1(@RequestParam("name") String name, 
									@RequestParam("page") int page, 
									Model model) {
	 * */
}
